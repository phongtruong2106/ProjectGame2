...
public class E1_MoveState : MoveState
{
    ...
    {
        base.LogicUpdate();
        //mang phat hien trong pham vi toi thieu
        if(isPlayerInMinArgoRange){
            //chuyen sang trang thai phat hien ( change State Deceted Player)
            stateMachine.ChangeState(enemy.playerDecetedState);
        }
        else if(isDetectingWall || !isDetectingLedge)
        {
            enemy.idleState.SetFlipAfterIdle(true);
            stateMachine.ChangeState(enemy.idleState); //chuyen no sang trang thai ma tui muon
        }
    }
